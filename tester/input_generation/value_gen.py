...
def generate_default(spec: TensorSpec, rng=NUMPY_RNG) -> object:
    ...
    if "int" in dtype:
        return rng.cast(rng.randint(-65535, 65535, size=shape), dtype)
    if dtype.startswith("complex"):
...
def generate_nonzero(spec: TensorSpec, rng=NUMPY_RNG) -> object:
    ...
    if "int" in dtype:
        ...
        if dtype == "uint8":
            return rng.cast(rng.randint(1, 256, size=shape), dtype)
        return rng.cast(rng.randint(1, 65535, size=shape), dtype)
    if dtype.startswith("complex"):
        real_dtype = "float32" if dtype == "complex64" else "float64"
...
def generate_fill_value(spec: TensorSpec, rng=NUMPY_RNG) -> object:
    ...
    dtype = generation_dtype(spec.dtype)
    if "int" in dtype:
        return rng.cast(rng.randint(1, 65535, size=spec.shape), dtype)
...
def generate_signed_half(spec: TensorSpec, rng=NUMPY_RNG) -> object:
    ...
    dtype = generation_dtype(spec.dtype)
    if "int" in dtype:
        return rng.cast(rng.randint(-65535, 65535, size=spec.shape), dtype)
    return rng.cast(rng.random(spec.shape) - 0.5, dtype)
...
def generate_normal_std(spec: TensorSpec, rng=NUMPY_RNG) -> object:
    ...
    dtype = generation_dtype(spec.dtype)
    if "int" in dtype:
        return rng.cast(rng.randint(0, 65535, size=spec.shape), dtype)
    return rng.cast(rng.random(spec.shape), dtype)


def generate_int_1024(spec: TensorSpec, rng=NUMPY_RNG) -> object:
    """生成整数区间 [0, 1024)。"""
    return rng.cast(rng.randint(0, 1024, size=spec.shape), generation_dtype(spec.dtype))


def generate_int_64(spec: TensorSpec, rng=NUMPY_RNG) -> object:
    """生成整数区间 [0, 64)。"""
    return rng.cast(rng.randint(0, 64, size=spec.shape), generation_dtype(spec.dtype))
...
def generate_int_128(spec: TensorSpec, rng=NUMPY_RNG) -> object:
    """生成整数区间 [1, 128)。"""
    return rng.cast(rng.randint(1, 128, size=spec.shape), generation_dtype(spec.dtype))


def generate_int_one_10(spec: TensorSpec, rng=NUMPY_RNG) -> object:
    """生成整数区间 [1, 10)。"""
    return rng.cast(rng.randint(1, 10, size=spec.shape), generation_dtype(spec.dtype))
...
def generate_empty_shape(spec: TensorSpec, rng=NUMPY_RNG) -> object:
    ...
    return rng.cast(rng.randint(1, 10, size=spec.shape), dtype)


def generate_int_2048(spec: TensorSpec, rng=NUMPY_RNG) -> object:
    """生成整数区间 [1, 2048)。"""
    return rng.cast(rng.randint(1, 2048, size=spec.shape), generation_dtype(spec.dtype))


def generate_int_65535(spec: TensorSpec, rng=NUMPY_RNG) -> object:
    """生成整数区间 [1, 65535)。"""
    return rng.cast(rng.randint(1, 65535, size=spec.shape), generation_dtype(spec.dtype))
...
def generate_int_or_default(spec: TensorSpec, rng=NUMPY_RNG) -> object:
    ...
    dtype = generation_dtype(spec.dtype)
    if "int" in dtype:
        return rng.cast(rng.randint(-127, 127, size=spec.shape), dtype)
    return generate_default(spec, rng)


def generate_binary01(spec: TensorSpec, rng=NUMPY_RNG) -> object:
    """生成二值标签 {0, 1}。"""
    return rng.cast(rng.randint(0, 2, size=spec.shape), generation_dtype(spec.dtype))
...
def generate_random_range(
    ...
    if "int" in dtype:
        ...
        return rng.cast(rng.randint(low, high, size=spec.shape), dtype)
    if dtype.startswith("complex"):
        real_dtype = "float32" if dtype == "complex64" else "float64"
        ...
        real = rng.cast(rng.uniform(real_low, real_high, size=spec.shape), real_dtype)
        imag = rng.cast(rng.uniform(real_low, real_high, size=spec.shape), real_dtype)
        return rng.cast(real + 1j * imag, dtype)
    low = low if low is not None else numpy.finfo(dtype).min / 2
    high = high if high is not None else numpy.finfo(dtype).max / 2
    return rng.cast(rng.uniform(low, high, size=spec.shape), dtype)
...
def generate_uniform(
    ...
    if dtype.startswith("complex"):
        real_dtype = "float32" if dtype == "complex64" else "float64"
        real = rng.cast(rng.uniform(low, high, size=spec.shape), real_dtype)
        imag = rng.cast(rng.uniform(low, high, size=spec.shape), real_dtype)
        return rng.cast(real + 1j * imag, dtype)
    return rng.cast(rng.uniform(low, high, size=spec.shape), dtype)
